...
let register_loc_name name = Hashtbl.replace used_attributes_tbl name.loc ()

let register_loc attr =
  !mark_as_handled_manually attr;
  register_loc_name attr.attr_name
...
let filter_extension key name =
  if name.txt = key then (
    register_loc_name name;
    true )
  else false
...
let id_of_expr = function
  ...
  | { pexp_desc = Pexp_construct ({ txt = Lident s; _ }, None); _ } ->
      s
...
let print_error ppf = function
  | Expression_expected -> Format.fprintf ppf "Expression expected"
  | Type_expected -> Format.fprintf ppf "Type expression expected"
  | Structure_expected -> Format.fprintf ppf "Structure expected"
  | Identifier_expected -> Format.fprintf ppf "String literal expected"
  | Invalid_expression -> Format.fprintf ppf "Invalid expression"
  | Multiple_binding_declarations ->
      Format.fprintf ppf "Multiple binding declarations"
  | Binding_type_mismatch ->
      Format.fprintf ppf "Binding declaration and type are not compatible"
  | Cannot_parse_type -> Format.fprintf ppf "Cannot parse type"
  | Cannot_parse_sigitem -> Format.fprintf ppf "Cannot parse signature item"
  | Cannot_parse_classdecl ->
      Format.fprintf ppf "Cannot parse class declaration"
  | Cannot_parse_classfield -> Format.fprintf ppf "Cannot parse class field"
  ...
  | Multiple_default_case ->
      Format.fprintf ppf
        "At most one default constructor is supported in variants"
  | Invalid_variadic_type_arg ->
      Format.fprintf ppf "A variadic function argument must be of type list"
  | No_input -> Format.fprintf ppf "An input file must be provided"
  | Multiple_inputs ->
      Format.fprintf ppf "A single input file must be provided"
  | Unlabelled_argument_in_builder ->
      Format.fprintf ppf "Arguments of builder must be named"
  | Spurious_attribute -> Format.fprintf ppf "Spurious js.* attribute"
  | Sum_kind_args ->
      Format.fprintf ppf
        "Incompatible label name for 'kind' and constructor arguments."
  | Constructor_in_union ->
...
let js_name ~global_attrs ?(capitalize = false) name =
  if has_attribute "js.verbatim_names" global_attrs then
    if capitalize then String.capitalize_ascii name else name
  else
...
let get_js_constr ~global_attrs name attributes =
  ...
  | Some (k, v) -> (
      match (expr_of_payload k.loc v).pexp_desc with
...
and arg = { lab : lab; att : attributes; typ : typ }
...
type methoddef = Getter of string | Setter of string | MethodCall of string
...
let rec parse_arg lab ~global_attrs ty =
  ...
  in
  { lab; att = ty.ptyp_attributes; typ = parse_typ ~global_attrs ty }

and parse_typ ~global_attrs ty =
  match ty.ptyp_desc with
  | Ptyp_arrow (lab, t1, t2)
    when has_attribute "js.variadic" t1.ptyp_attributes -> (
      match parse_arg lab ~global_attrs t1 with
      | { lab; att; typ = Name ("list", [ typ ]) } -> (
          let ty_vararg = Some { lab; att; typ } in
          match parse_typ ~global_attrs t2 with
          | Arrow
              ( { ty_args = []; ty_vararg = None; unit_arg = _; ty_res = _ } as
              params )
            when t2.ptyp_attributes = [] ->
              Arrow { params with ty_vararg }
          | Arrow _ when t2.ptyp_attributes = [] ->
              error ty.ptyp_loc Cannot_parse_type
          | tres ->
              Arrow
                { ty_args = []; ty_vararg; unit_arg = false; ty_res = tres } )
      | _ -> error t1.ptyp_loc Invalid_variadic_type_arg )
  | Ptyp_arrow (lab, t1, t2) -> (
      let t1 = parse_arg lab ~global_attrs t1 in
      match parse_typ ~global_attrs t2 with
      | Arrow ({ ty_args; ty_vararg = _; unit_arg = _; ty_res = _ } as params)
        when t2.ptyp_attributes = [] ->
          Arrow { params with ty_args = t1 :: ty_args }
      | tres -> (
          match t1 with
          | { lab = Arg; att = []; typ = Unit _ } ->
              Arrow
                {
                  ty_args = [];
                  ty_vararg = None;
  ...
  | Ptyp_constr ({ txt = lid; loc = _ }, tl) -> (
      match (String.concat "." (Longident.flatten lid), tl) with
  ...
  | Ptyp_variant (rows, Closed, None) ->
      ...
      let prepare_row = function
        | {
            prf_desc = Rtag ({ txt = mlconstr; _ }, true, []);
            prf_attributes = attributes;
            prf_loc = location;
          } ->
            { mlconstr; arg = Constant; attributes; location }
        | {
            prf_desc = Rtag ({ txt = mlconstr; _ }, false, [ typ ]);
            prf_attributes = attributes;
            prf_loc = location;
          } -> (
            match parse_typ ~global_attrs typ with
      ...
      Variant
        {
          ...
          constrs = List.map prepare_row rows;
        }
  | Ptyp_tuple typs ->
...
let check_prefix ~prefix s =
  ...
  if l <= String.length s && String.sub s 0 l = prefix then
...
let check_suffix ~suffix s =
  ...
  if l <= String.length s && String.sub s (String.length s - l) l = suffix then
...
let in_global_scope ~global_attrs js =
  match
    choose (fun x -> get_string_attribute "js.scope" [ x ]) global_attrs
  with
  | [] -> js
...
let auto ~global_attrs s ty =
  let methcall s =
    ...
    if has_attribute "js.scope" global_attrs then
      Global (in_global_scope ~global_attrs js)
    else MethCall js
  in
  match ty with
  | Arrow
      {
        ty_args = [ { lab = Arg; att = _; typ = Name (t, []) } ];
        ty_vararg = None;
        unit_arg = false;
        ty_res = Js;
      }
    when check_suffix ~suffix:"_to_js" s = Some t ->
      Cast
  | Arrow
      {
        ty_args = [ { lab = Arg; att = _; typ = Js } ];
        ty_vararg = None;
        unit_arg = false;
        ty_res = Name (t, []);
      }
    when check_suffix ~suffix:"_of_js" s = Some t ->
      Cast
  | Arrow
      { ty_args = [ _ ]; ty_vararg = None; unit_arg = false; ty_res = Unit _ }
    when has_prefix ~prefix:"set_" s ->
      PropSet
        (in_global_scope ~global_attrs
           (js_name ~global_attrs (drop_prefix ~prefix:"set_" s)))
  | Arrow
      {
        ty_args = [ { lab = Arg; att = _; typ = Name _ } ];
        ty_vararg = None;
        unit_arg = false;
        ty_res = Unit _;
      } ->
  ...
  | Arrow
      ...
      PropGet (js_name ~global_attrs s)
  | Arrow { ty_args = []; ty_vararg = None; unit_arg = true; ty_res = _ } ->
      PropGet (in_global_scope ~global_attrs (js_name ~global_attrs s))
  | Arrow
      {
        ty_args = [ { lab = Arg; att = _; typ = Name _ }; _ ];
        ty_vararg = None;
        unit_arg = false;
        ty_res = Unit _;
      }
  ...
  | Arrow { ty_args = _; ty_vararg = None; unit_arg = false; ty_res = Name _ }
    when has_prefix ~prefix:"new_" s ->
      New
        (in_global_scope ~global_attrs
           (js_name ~global_attrs (drop_prefix ~prefix:"new_" s)))
  | Arrow
      {
        ty_args = { lab = Arg; att = _; typ = Name _ } :: _;
        ty_vararg = _;
  ...
  let opt_name ?(prefix = "") ?(capitalize = false) ?(global = false) () =
    ...
    | PStr [] -> (
        ...
        | Some s ->
            ...
            if global then in_global_scope ~global_attrs js else js )
    | _ ->
        id_of_expr
          (expr_of_payload attribute.attr_name.loc attribute.attr_payload)
  in
  let actions =
    [
      ("js.cast", fun () -> Cast);
      ...
      ( "js.new",
        fun () ->
          New (opt_name ~prefix:"new_" ~global:true ~capitalize:true ()) );
      ("js.builder", fun () -> Builder global_attrs);
    ]
  in
  match
    List.find (fun (name, _) -> filter_attr_name name attribute) actions
  with
  | exception Not_found -> None
  | _, f -> Some (f ())
...
let rec parse_sig_item ~global_attrs rest s =
  ...
  | Psig_type (rec_flag, decls) -> Type (rec_flag, decls) :: rest
  | Psig_module
      {
        pmd_name;
        pmd_type =
          { pmty_desc = Pmty_signature si; pmty_attributes; pmty_loc = _ };
        pmd_loc = _;
        pmd_attributes = _;
      } ->
      ...
      :: rest
  | Psig_class cs ->
      Class (List.map (parse_class_decl ~global_attrs) cs) :: rest
  | Psig_attribute ({ attr_payload = PStr str; _ } as attribute)
    when filter_attr_name "js.implem" attribute ->
      Implem str :: rest
...
and parse_sig ~global_attrs = function
  | [] -> []
  | { psig_desc = Psig_attribute attribute; _ } :: rest
    when filter_attr_name "js.stop" attribute ->
      parse_sig_verbatim ~global_attrs rest
  | { psig_desc = Psig_value vd; _ } :: rest
    when has_attribute "js.custom" vd.pval_attributes ->
  ...
  | s :: rest -> parse_sig_item ~global_attrs (parse_sig ~global_attrs rest) s

and parse_sig_verbatim ~global_attrs = function
  | [] -> []
  | { psig_desc = Psig_attribute attribute; _ } :: rest
    when filter_attr_name "js.start" attribute ->
      parse_sig ~global_attrs rest
              ...
              ( Nolabel,
                {
                  ptyp_desc =
                    Ptyp_constr
                      ( { txt = Longident.Ldot (Lident "Ojs", "t"); loc = _ },
                        [] );
                  ptyp_loc = _;
                  ptyp_attributes = _;
                  ptyp_loc_stack = _;
                },
                {
                  pcty_desc =
                    Pcty_signature
                      {
                        pcsig_self = { ptyp_desc = Ptyp_any; _ };
                        pcsig_fields;
                      };
                  pcty_loc = _;
                  pcty_attributes = _;
      ...
      Declaration
        {
          class_name;
          class_fields =
            List.map (parse_class_field ~global_attrs) pcsig_fields;
        }
  | {
      pci_virt = Concrete;
      pci_params = [];
      ...
      let rec convert_typ = function
        ...
        | {
            pcty_desc = Pcty_arrow (label, typ, ct);
            pcty_attributes;
            pcty_loc;
          } ->
            Typ.arrow ~loc:pcty_loc ~attrs:pcty_attributes label typ
              (convert_typ ct)
      ...
      let class_arrow =
        ...
        | Arrow
            ...
            params
        | Name (_, []) as ty_res ->
            { ty_args = []; ty_vararg = None; unit_arg = false; ty_res }
        | _ -> error pci_loc Cannot_parse_classdecl
      in
      let class_name = pci_name.txt in
      let js_class_name =
...
and parse_class_field ~global_attrs = function
  | {
      pctf_desc = Pctf_method ({ txt = method_name; _ }, Public, Concrete, typ);
      pctf_loc;
      pctf_attributes;
    } ->
      ...
      let defs =
        choose
          (parse_attr ~global_attrs (method_name, pctf_loc, auto))
          pctf_attributes
      in
      let kind =
        match defs with
      ...
      in
      Method
        { method_name; method_typ = ty; method_def; method_loc = pctf_loc }
...
let list_iter f x =
  Exp.apply
    (Exp.ident (mknoloc (Longident.parse "List.iter")))
    (nolabel [ f; x ])
...
let fun_ (label, s) e =
  ...
  | Pexp_apply
      ( f,
        [
          (Nolabel, { pexp_desc = Pexp_ident { txt = Lident x; loc = _ }; _ });
        ] )
    when x = s ->
      f
  | _ -> Exp.fun_ label None (Pat.var (mknoloc s)) e
...
let fun_unit e =
  ...
  | Pexp_apply
      ...
      f
  | _ -> Exp.fun_ Nolabel None (Pat.construct (mknoloc (Lident "()")) None) e
...
let some_pat arg = Pat.construct (mknoloc (Longident.parse "Some")) (Some arg)

let none_pat () = Pat.construct (mknoloc (Longident.parse "None")) None
...
let exp_ignore res = apply (var "ignore") [ (Nolabel, res) ]
...
let def s ty body =
  Str.value Nonrecursive
    [ Vb.mk (Pat.constraint_ (Pat.var (mknoloc s)) ty) body ]

let builtin_type = function
  | "int" | "string" | "bool" | "float" | "array" | "list" | "option" -> true
  | _ -> false
...
let assert_false =
  Exp.assert_ (Exp.construct (mknoloc (Longident.parse "false")) None)
...
let is_simple_enum params =
  ...
  let p { mlconstr = _; arg; attributes; location = _ } =
    ...
    | Unary _ | Nary _ | Record _ -> false
  in
  List.for_all p params

type union_discriminator = No_discriminator | On_field of string

let get_variant_kind loc attrs =
  if has_attribute "js.enum" attrs then `Enum
  else if has_attribute "js.union" attrs then
    ...
    | Some (k, v) -> (
        ...
        | _ -> (
            match expr_of_payload k.loc v with
            | {
             pexp_desc =
               Pexp_apply
                 ( { pexp_desc = Pexp_ident { txt = Lident "on_field"; _ }; _ },
                   [
                     ( Nolabel,
                       { pexp_desc = Pexp_constant (Pconst_string (s, _)); _ }
                     );
  ...
  else if has_attribute "js.sum" attrs then
  ...
  else error loc (Not_supported_here "Sum types without js.* attribute")
...
let rec js2ml ty exp =
  ...
  | Unit loc -> error loc (Not_supported_here "Unit")
  | Variant { location; global_attrs; attributes; constrs } ->
      js2ml_of_variant ~variant:true location ~global_attrs attributes constrs
        exp
  | Tuple typs ->
      let f x =
        Exp.tuple
          (List.mapi
             (fun i typ -> js2ml typ (ojs "array_get" [ x; int i ]))
             typs)
      in
      let_exp_in exp f
...
and js2ml_of_variant ~variant loc ~global_attrs attrs constrs exp =
  ...
  let check_label =
    ...
    | `Sum kind ->
        fun loc label -> if label = kind then error loc Sum_kind_args
    | _ -> fun _ _ -> ()
  in
  let mkval =
  ...
  in
  let f exp =
    let gen_cases (int_default, int_cases, string_default, string_cases)
        { mlconstr; arg; attributes; location } =
      let case x =
        ...
        | `String s ->
            ( int_default,
              int_cases,
              string_default,
              Exp.case (pat_str s) x :: string_cases )
        | `Int n ->
            ( int_default,
              Exp.case (pat_int n) x :: int_cases,
              string_default,
              string_cases )
      in
      ...
      | Unary arg_typ -> (
          let otherwise () =
            ...
            | `Sum _ ->
                let loc, arg_field =
                  get_string_attribute_default "js.arg" (location, "arg")
                    attributes
                in
                ...
                if List.for_all (( = ) None) defs then
                  ...
                  | `Enum ->
                      ...
                      cont
                        (Exp.case
                           (Pat.var (mknoloc x))
                           (mkval mlconstr (Some (var x))))
                  | `Sum _ | `Union _ ->
                      cont
                        (Exp.case (Pat.any ())
                           (mkval mlconstr (Some (js2ml arg_typ exp))))
                else error k.loc Multiple_default_case
          in
          match variant_kind with
          | `Enum when arg_typ = int_typ ->
              process_default [ int_default ] (fun int_default ->
                  (Some int_default, int_cases, string_default, string_cases))
          | `Enum when arg_typ = string_typ ->
              process_default [ string_default ] (fun string_default ->
                  (int_default, int_cases, Some string_default, string_cases))
          | (`Sum _ | `Union _) when arg_typ = Js ->
              process_default [ int_default; string_default ] (fun default ->
                  (Some default, int_cases, Some default, string_cases))
          | _ -> otherwise () )
      | Nary args_typ -> (
          match variant_kind with
          | `Enum -> error location Non_constant_constructor_in_enum
          | `Sum _ ->
              ...
              let get_args key i typ =
                js2ml typ
                  (ojs "array_get" [ ojs "get" [ exp; str key ]; int i ])
              in
              case
                (mkval mlconstr
                   (Some (Exp.tuple (List.mapi (get_args args_field) args_typ))))
              ...
              case
                (mkval mlconstr
                   (Some
                      (Exp.record
                         (List.map
                            (fun (loc, mlname, jsname, typ) ->
                              check_label loc jsname;
                              (mlname, get_arg jsname typ))
                            args)
                         None)))
          | `Union _ -> error location Constructor_in_union )
    in
    let int_default, int_cases, string_default, string_cases =
      List.fold_left gen_cases (None, [], None, []) constrs
    in
    ...
    in
    let int_match =
      gen_match (js2ml int_typ discriminator) int_default int_cases
    in
    let string_match =
      gen_match (js2ml string_typ discriminator) string_default string_cases
    in
    ...
    | Some int_match, Some string_match ->
        ...
        let case_default =
          ...
          | `Enum, _, _ | _, None, None -> Exp.case (Pat.any ()) assert_false
          | (`Sum _ | `Union _), Some def1, Some def2 ->
              assert (def1 = def2);
              def1
          | (`Sum _ | `Union _), Some _, None
          | (`Sum _ | `Union _), None, Some _ ->
              assert false
        in
        Exp.match_
          (ojs "type_of" [ discriminator ])
          [ case_int; case_string; case_default ]
  in
  let_exp_in exp f
...
and ml2js ty exp =
  ...
  | Arrow { ty_args; ty_vararg = None; unit_arg; ty_res } ->
      ...
      let formal_args, concrete_args =
        (List.map fst args, List.map snd args)
      in
      let res = ml2js_unit ty_res (app exp concrete_args unit_arg) in
      let body =
        if formal_args = [] then Exp.fun_ Nolabel None (Pat.any ()) res
        else res
      in
  ...
  | Arrow
      {
        ty_args;
        ty_vararg = Some { lab = label_variadic; att = _; typ = ty_variadic };
        unit_arg;
        ty_res;
      } ->
      ...
      let concrete_args =
        List.mapi
          (fun i { lab; att = _; typ } ->
            ( arg_label lab,
              js2ml typ (ojs "array_get" [ var arguments; int i ]) ))
          ty_args
      in
      let extra_arg =
        ojs "list_of_js_from"
          [ js2ml_fun ty_variadic; var arguments; int n_args ]
      in
      let extra_arg =
        match label_variadic with
        | Arg | Lab _ -> extra_arg
        | Opt _ ->
            Exp.construct (mknoloc (Longident.parse "Some")) (Some extra_arg)
      in
      let concrete_args =
        concrete_args @ [ (arg_label label_variadic, extra_arg) ]
      in
  ...
  | Unit loc -> error loc (Not_supported_here "Unit")
  | Variant { location; global_attrs; attributes; constrs } ->
      ml2js_of_variant ~variant:true location ~global_attrs attributes constrs
        exp
  | Tuple typs ->
      ...
      Exp.let_ Nonrecursive [ Vb.mk pat exp ]
        (let n = List.length typs in
         ...
         Exp.let_ Nonrecursive
           [ Vb.mk (Pat.var (mknoloc a)) new_array ]
           (let f e (i, typ, x) =
              Exp.sequence
                (ojs "array_set" [ var a; int i; ml2js typ (var x) ])
                e
            in
...
and ml2js_of_variant ~variant loc ~global_attrs attrs constrs exp =
  ...
  let case { mlconstr; arg; attributes; location } =
    let mkobj args =
      ...
      | `Sum kind, _ ->
          ojs "obj"
            [ Exp.array (Exp.tuple [ str kind; discriminator ] :: args) ]
      | `Union _, [] -> ojs_null
    ...
    | Unary arg_typ ->
        ...
        let value =
          match variant_kind with
          | `Enum
            when (arg_typ = int_typ || arg_typ = string_typ)
                 && has_attribute "js.default" attributes ->
              ml2js arg_typ (var x)
          | `Enum | `Sum _ ->
              let loc, arg_field =
    ...
    | Nary args_typ ->
        let loc, args_field =
          get_string_attribute_default "js.arg" (location, "arg") attributes
        in
        check_label loc args_field;
        ...
        Exp.case
          (mkpat mlconstr
             (Some
                (Pat.tuple
                   (List.map (fun (_, _, xi) -> Pat.var (mknoloc xi)) xis))))
          (let args = fresh () in
           Exp.let_ Nonrecursive
             [
             ...
             (List.fold_left
                (fun e (i, typi, xi) ->
                  Exp.sequence
                    (ojs "array_set" [ var args; int i; ml2js typi (var xi) ])
                    e)
        ...
        Exp.case
          ...
          (mkobj
             (List.map
                (fun (loc, mlname, jsname, typ) ->
                  check_label loc jsname;
                  pair jsname typ (Exp.field (var x) mlname))
                args))
  in
    ...
    && List.for_all
         (function
           | {
               typ =
                 Variant
                   { location = _; global_attrs = _; attributes; constrs };
               _;
             }
             when has_attribute "js.enum" attributes ->
...
and prepare_args_push ty_args ty_vararg =
  let push arr typ x =
    match typ with
    | Variant { location = _; global_attrs; attributes; constrs }
      when has_attribute "js.enum" attributes && not (is_simple_enum constrs)
      ->
        let f { mlconstr; arg; attributes; location = _ } =
          ...
          let gen_tuple typs =
            ...
            let pat =
              ...
              | _ :: _ :: _ ->
                  Pat.variant mlconstr
                    (Some
                       (Pat.tuple
                          (List.map (fun (_, xi) -> Pat.var (mknoloc xi)) xis)))
            in
            Exp.case pat
              (let args =
                 mkargs (List.map (fun (typi, xi) -> ml2js typi (var xi)) xis)
               in
               match args with
               | [] -> unit_expr
               | _ :: _ ->
    ...
    | typ ->
        exp_ignore (ojs "call" [ arr; str "push"; Exp.array [ ml2js typ x ] ])
  in
  let f { lab; att = _; typ } =
    let s = fresh () in
    ( (arg_label lab, s),
      fun arr ->
        ...
        | Opt { def; _ } -> (
            match def with
            | None ->
                match_some_none ~none:unit_expr
                  ~some:(fun s -> push arr typ s)
                  s
            | Some none ->
        ...
        ( formal_args @ [ (arg_label lab, arg) ],
          ...
          @ [
              (fun arr ->
                ...
                | Opt _ ->
                    match_some_none ~none:unit_expr ~some:extra_args (var arg));
            ] )
  in
  let body arr =
    List.fold_right (fun code -> Exp.sequence (code arr)) concrete_args arr
  in
  ( formal_args,
...
and gen_typ = function
  ...
  | Js -> ojs_typ
  | Unit _ -> Typ.constr (mknoloc (Lident "unit")) []
  | Arrow { ty_args; ty_vararg; unit_arg; ty_res } ->
      let tl =
        match ty_vararg with
        | None -> ty_args
        | Some { lab; att; typ } ->
            ty_args @ [ { lab; att; typ = Name ("list", [ typ ]) } ]
      in
      let tl =
        if unit_arg then tl @ [ { lab = Arg; att = []; typ = Unit none } ]
        else tl
      in
      List.fold_right
        (fun { lab; att = _; typ } t2 ->
          Typ.arrow (arg_label lab) (gen_typ typ) t2)
        tl (gen_typ ty_res)
  ...
  | Tuple typs -> Typ.tuple (List.map gen_typ typs)
...
and gen_funs ~global_attrs p =
  ...
  let of_js, to_js =
    ...
    | _, Ptype_variant cstrs ->
        ...
        ( lazy
            (mkfun
               (js2ml_of_variant ~variant:false loc ~global_attrs attrs params)),
          lazy
            (mkfun
               (ml2js_of_variant ~variant:false loc ~global_attrs attrs params))
        )
    | _, Ptype_record lbls ->
        let global_attrs = p.ptype_attributes @ global_attrs in
        let lbls = List.map (process_fields ~global_attrs) lbls in
        let of_js x (_loc, ml, js, ty) =
          (ml, js2ml ty (ojs "get" [ x; str js ]))
        in
        let to_js x (_loc, ml, js, ty) =
          Exp.tuple [ str js; ml2js ty (Exp.field x ml) ]
        in
  ...
  let force_opt x =
    ...
    with Error (_, Union_without_discriminator) -> None
  in
          ...
          [
            Vb.mk
              ...
              (Exp.newtype (mknoloc v)
                 (Exp.constraint_ to_js
                    (Typ.arrow Nolabel
                       (Typ.constr
                          (mknoloc (Longident.parse name))
                          [ Typ.constr (mknoloc (Longident.parse v)) [] ])
                       ojs_typ)));
          ] )
  | _ ->
      ...
      choose f
        [
          (name ^ "_of_js", Js, Name (name, []), of_js);
...
and gen_decl ~global_attrs = function
  ...
  | Module (s, attrs, decls) ->
      ...
      [
        Str.module_
          (Mb.mk (mknoloc s) (Mod.structure (gen_decls ~global_attrs decls)));
      ]
  | Val (s, ty, decl, loc) ->
  ...
  | Implem str -> mapper.Ast_mapper.structure mapper str
  | Open descr ->
...
and gen_classdecl cast_funcs = function
  | Declaration { class_name; class_fields } ->
      ...
      let ign = function
        | { pvb_pat = { ppat_desc = Ppat_var { txt; loc = _ }; _ }; _ } ->
            Vb.mk (Pat.any ()) (var txt)
        | _ -> assert false
      in
      let obj = Cl.let_ Nonrecursive (List.map ign cast_funcs) obj in
      let obj = Cl.let_ Nonrecursive cast_funcs obj in
      Ci.mk (mknoloc class_name)
        (Cl.fun_ Nolabel None
           (Pat.constraint_ (Pat.var (mknoloc x)) ojs_typ)
           obj)
  | Constructor
      {
        class_name;
        js_class_name;
        class_arrow = { ty_args; ty_vararg; unit_arg; ty_res };
      } ->
      let formal_args, concrete_args = prepare_args ty_args ty_vararg in
      ...
      let e =
        Cl.apply
          (Cl.constr (mknoloc (Longident.parse super_class)) [])
          [ (Nolabel, obj) ]
      in
...
and gen_class_field x = function
  | Method { method_name; method_typ; method_def; method_loc } ->
      ...
      in
      Cf.method_ (mknoloc method_name) Public
        (Cf.concrete Fresh (Exp.constraint_ body (gen_typ method_typ)))
  | Inherit super ->
...
and gen_class_cast = function
  | Declaration { class_name; class_fields = _ } ->
      ...
      let to_js =
        let arg = fresh () in
        Vb.mk
          (Pat.var (mknoloc (class_name ^ "_to_js")))
      ...
      let of_js =
        let arg = fresh () in
        Vb.mk
          (Pat.var (mknoloc (class_name ^ "_of_js")))
          (Exp.fun_ Nolabel None
             (Pat.constraint_ (Pat.var (mknoloc arg)) ojs_typ)
             (Exp.constraint_
                (Exp.apply
                   (Exp.new_ (mknoloc (Longident.Lident class_name)))
                   [ (Nolabel, var arg) ])
                class_typ))
      in
      [ to_js; of_js ]
...
and gen_def loc decl ty =
  ...
  | ( PropGet s,
      Arrow { ty_args = []; ty_vararg = None; unit_arg = true; ty_res } ) ->
  ...
  | ( PropSet s,
      Arrow
        {
          ty_args =
            [
              { lab = Arg; att = _; typ = Name _ as ty_this };
      ...
      let res this arg =
        ojs "set" [ ml2js ty_this this; str s; ml2js ty_arg arg ]
      in
      mkfun (fun this -> mkfun (fun arg -> res this arg))
  | ( PropSet s,
      Arrow
        {
          ty_args = [ { lab = Arg; att = _; typ = ty_arg } ];
          ty_vararg = None;
          unit_arg = false;
          ty_res = Unit _;
        } ) ->
  ...
  | ( MethCall s,
      Arrow
        {
          ty_args = { lab = Arg; att = _; typ } :: ty_args;
          ty_vararg;
          unit_arg;
          ty_res;
        } ) ->
  ...
  | Builder global_attrs, Arrow { ty_args; ty_vararg = None; unit_arg; ty_res }
    ->
        ...
        ( (arg_label lab, s),
          fun x ->
            let js =
              ...
              | None, (Lab { ml; _ } | Opt { ml; _ }) ->
                  js_name ~global_attrs ml
            in
            ...
            | Opt { def; _ } -> (
                match def with
                | None -> match_some_none (var s) ~none:unit_expr ~some:code
                | Some none ->
      ...
      let init x =
        List.fold_left (f x) (js2ml_unit ty_res x) (List.rev concrete_args)
      in
      let body = let_exp_in (ojs "empty_obj" [ unit_expr ]) init in
      func formal_args unit_arg body
  | _ -> error loc Binding_type_mismatch
...
and str_of_sg ~global_attrs sg =
  ...
  let attr =
    attr "ocaml.text" (str "!! This code has been generated by gen_js_api !!")
  in
  register_loc attr;
  Str.attribute attr :: disable_warnings :: gen_decls ~global_attrs decls
...
and mapper =
  ...
  and module_expr self mexp =
    ...
    | Pmod_constraint
        ( { pmod_desc = Pmod_extension (attr, PStr []); _ },
          ( { pmty_desc = Pmty_signature sg; pmty_attributes; pmty_loc = _ } as
          mty ) )
      when filter_extension "js" attr ->
        Mod.constraint_
          (Mod.structure (str_of_sg ~global_attrs:pmty_attributes sg))
          mty
    | _ -> mexp
  in
  let structure_item self str =
    ...
    | Pstr_primitive vd when vd.pval_prim = [] -> (
        match parse_valdecl ~global_attrs ~in_sig:false vd with
        | exception Exit -> str
        | d -> incl (gen_decls ~global_attrs [ d ]) )
    | Pstr_type (rec_flag, decls) -> (
        let js_decls =
          List.filter (fun d -> has_attribute "js" d.ptype_attributes) decls
        in
        match js_decls with
        | [] -> str
        | l ->
            with_default_loc str.pstr_loc (fun () ->
                      ...
                      pstr_desc =
                        Pstr_type
                          ( rec_flag,
                            List.map
                              (fun d ->
                                if has_attribute "js" d.ptype_attributes then
                                  rewrite_typ_decl d
                                else d)
                              decls );
                    };
  ...
  let expr self e =
    ...
    | Pexp_extension (attr, PTyp ty) when filter_extension "js.to" attr ->
        with_default_loc e.pexp_loc (fun () ->
            js2ml_fun (parse_typ ~global_attrs ty))
    | Pexp_extension (attr, PTyp ty) when filter_extension "js.of" attr ->
        with_default_loc e.pexp_loc (fun () ->
            ml2js_fun (parse_typ ~global_attrs ty))
    | _ -> e
  in
...
let standalone () =
  ...
  let sg = Pparse.parse_interface ~tool_name:"gen_js_iface" src in
...
let mapper =
  {
    ...
    Ast_mapper.structure =
      (fun _this str ->
        check_loc_mapper.Ast_mapper.structure check_loc_mapper
